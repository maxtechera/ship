...
#   bash ~/.claude/skills/credentials/scripts/auth.sh --only linear
...
whoami_openai() {
  local t; t=$(_token OPENAI_API_KEY "$CLAW_DIR/.openai_token")
...
whoami_anthropic() {
  local t; t=$(_token ANTHROPIC_API_KEY "$CLAW_DIR/.anthropic_token")
  [[ -z "$t" ]] && { echo "need_auth|no key"; return; }
...
whoami_gemini() {
  local t; t=$(_token GEMINI_API_KEY "$CLAW_DIR/.gemini_token")
  [[ -z "$t" ]] && { echo "need_auth|no key"; return; }
...
whoami_xai() {
  local t; t=$(_token XAI_API_KEY "$CLAW_DIR/.xai_token")
  [[ -z "$t" ]] && { echo "need_auth|no key"; return; }
...
whoami_perplexity() {
  local t; t=$(_token PERPLEXITY_KEY "$CLAW_DIR/.perplexity_token")
...
whoami_meta() {
  local t; t=$(_token META_ACCESS_TOKEN "$CLAW_DIR/.meta_token")
...
whoami_telegram() {
  local t; t=$(_token TELEGRAM_BOT_TOKEN "$CLAW_DIR/.telegram_bot_token")
  [[ -z "$t" ]] && { echo "need_auth|no token"; return; }
...
whoami_brave() {
  local t; t=$(_token BRAVE_API_KEY "$CLAW_DIR/.brave_token")
  [[ -z "$t" ]] && { echo "need_auth|no key"; return; }
...
whoami_manychat() {
  local t; t=$(_token MANYCHAT_TOKEN "$CLAW_DIR/.manychat_token")
  [[ -z "$t" ]] && { echo "need_auth|no token"; return; }
...
whoami_zoom() {
  local acct cid csec
  acct=$(cat "$CLAW_DIR/.zoom_account_id" 2>/dev/null | tr -d '\n')
  cid=$(cat "$CLAW_DIR/.zoom_client_id" 2>/dev/null | tr -d '\n')
  csec=$(cat "$CLAW_DIR/.zoom_client_secret" 2>/dev/null | tr -d '\n')
...
whoami_ga4() {
  ...
  local prop="${GA4_PROPERTY_ID:-502068431}"
  ...
  if echo "$body" | grep -q '"rows"\|"rowCount"'; then
    ...
    echo "need_auth|GA4 403 — add analytics.readonly scope to OAuth client, then: gog auth add maxi.techerag@gmail.com --force-consent"
  else
...
whoami_gsc() {
  ...
  if echo "$body" | grep -q "siteEntry"; then
    ...
    echo "need_auth|GSC 403 — add webmasters.readonly scope to OAuth client, then: gog auth add maxi.techerag@gmail.com --force-consent"
  else
...
whoami_mercadopago() {
  local t; t=$(_token MERCADOPAGO_ACCESS_TOKEN "$CLAW_DIR/.mercadopago_token")
  [[ -z "$t" ]] && { echo "need_auth|no token"; return; }
...
whoami_google_ads() {
  local t; t=$(_gog_access_token)
  [[ -z "$t" ]] && { echo "need_auth|gog not authenticated"; return; }
  local dev_token; dev_token=$(cat "$CLAW_DIR/.google_ads_dev_token" 2>/dev/null | tr -d '\n')
  if [[ -z "$dev_token" ]]; then
    # No dev token — check if adwords scope is at least in the OAuth token
    local scopes; scopes=$(gog auth tokens export maxi.techerag@gmail.com --out /tmp/.gog_ads_check.json --overwrite >/dev/null 2>&1 && python3 -c "
import json, urllib.request, urllib.parse, os
...
whoami_mercadolibre() {
  # Separate app + token from MercadoPago
  local t=""
  if [[ -f "$CLAW_DIR/.mercadolibre.json" ]]; then
    t=$(python3 -c "import json; print(json.load(open('$CLAW_DIR/.mercadolibre.json'))['access_token'])" 2>/dev/null)
  fi
  [[ -z "$t" ]] && { echo "need_auth|no token ($CLAW_DIR/.mercadolibre.json)"; return; }
...
whoami_notion() {
  ...
  local t; t=$(_token NOTION_API_KEY "$CLAW_DIR/.notion_token")
  if [[ -n "$t" ]]; then
...
# auth_type: cli (run command) | token (paste + save to file)
ENTRIES=(
  # ── Platform CLIs ──
  ...
  "openai;whoami_openai;token;$CLAW_DIR/.openai_token|sk-|https://platform.openai.com/api-keys;OpenAI"
  "anthropic;whoami_anthropic;token;$CLAW_DIR/.anthropic_token|sk-ant-|https://console.anthropic.com/settings/keys;Anthropic"
  "gemini;whoami_gemini;token;$CLAW_DIR/.gemini_token||https://aistudio.google.com/apikey;Google Gemini"
  "xai;whoami_xai;token;$CLAW_DIR/.xai_token|xai-|https://console.x.ai;xAI / Grok"
  "perplexity;whoami_perplexity;token;$CLAW_DIR/.perplexity_token|pplx-|https://www.perplexity.ai/settings/api;Perplexity"
  "meta;whoami_meta;token;$CLAW_DIR/.meta_token|EAA|https://developers.facebook.com/tools/explorer;Meta / Facebook"
  ...
  "mercadopago;whoami_mercadopago;token;$CLAW_DIR/.mercadopago_token||https://www.mercadopago.com/developers;MercadoPago"
  "mercadolibre;whoami_mercadolibre;token;$CLAW_DIR/.mercadolibre.json||https://developers.mercadolibre.com.ar/devcenter;MercadoLibre (OAuth — 6h token + refresh)"
  "telegram;whoami_telegram;token;$CLAW_DIR/.telegram_bot_token||@BotFather on Telegram;Telegram Bot"
  "brave;whoami_brave;token;$CLAW_DIR/.brave_token||https://brave.com/search/api;Brave Search"
  "manychat;whoami_manychat;token;$CLAW_DIR/.manychat_token||https://manychat.com/settings/api;ManyChat"
...
for entry in "${need_auth_entries[@]}"; do
  ...
  case "$auth_type" in
    ...
    zoom)
      ...
      _save_token "$CLAW_DIR/.zoom_account_id" "$z_acct"
      _save_token "$CLAW_DIR/.zoom_client_id" "$z_cid"
      _save_token "$CLAW_DIR/.zoom_client_secret" "$z_csec"
      echo -e "  Saved 3 credentials to $CLAW_DIR/.zoom_*"
      echo ""
      result=$($whoami_fn)
      local_status="${result%%|*}"
      local_id="${result#*|}"
...
echo -e "${BOLD}═══════════════════════════════════════════════════════${RESET}"
echo -e "  Done! Run ${BOLD}/credentials verify${RESET} for full status."
echo ""
