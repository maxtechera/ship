...
def check_meta():
    ...
    token_path = Path(CLAW_DIR) / ".meta_token"
...
def refresh_meta(check_only=False):
    ...
    # Attempt to extend
    app_secret_path = Path(CLAW_DIR) / ".meta_app_secret"
    app_id_path = Path(CLAW_DIR) / ".meta_app_id"
    ...
    token = (Path(CLAW_DIR) / ".meta_token").read_text().strip()
    code, body = http_get(
...
def check_mercadolibre():
    ...
    ml_path = Path(CLAW_DIR) / ".mercadolibre.json"
...
def refresh_mercadopago(check_only=False):
    ...
    name = "mercadopago"
    token_path = Path(CLAW_DIR) / ".mercadopago_token"
